...
//self->root = pf_kdtree_insert_node(self, NULL, self->root, key, value);
////////////////////////////////////////////////////////////////////////////////
// Insert a node into the tree//这里定义的kd_tree只在叶子节点存放数据信息(key)(中间节点和根节点有效的是分割信息)
                                        ...
                                        pf_kdtree_node_t *node, int key[], double value)
{
  ...
  if (node == NULL)
  {
    assert(self->node_count < self->node_max_count);//若大于最大节点数,则终止程序
...
{
  if (node->leaf)//查找到的必须是叶子节点,只有叶子节点的key才是有效的
  {
...
void pf_kdtree_cluster(pf_kdtree_t *self)
{
  ...
  for (i = 0; i < self->node_count; i++)//将所有叶子节点存于queue,并所有节点聚类标签置为-1;
  {
    node = self->nodes + i;
    if (node->leaf)
    {
...
void pf_kdtree_cluster_node(pf_kdtree_t *self, pf_kdtree_node_t *node, int depth)
{
  ...
  for (i = 0; i < 3 * 3 * 3; i++)//将(x,y,theta)视为一个三维空间,将同处于3*3*3的立方体范围内的点视为同一类,
  {
      ...
      pf_kdtree_cluster_node(self, nnode, depth + 1);//递归调用,找与 nnode(与node同一类的) 同一类的节点
                                                     //这样可以将空间上连成一片的点聚为一类
  }
  return;
}
