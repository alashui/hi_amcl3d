...
//从参数服务器中更新机器人位姿//"initial_pose_x"...
  ...
  laser_scan_filter_ = new tf::MessageFilter<sensor_msgs::LaserScan>(*laser_scan_sub_,
                                                                      *tf_, 
                                                                      odom_frame_id_, 
                                                                      100);
  //tf::MessageFilter的作用：把接受到的激光数据消息变换到指定的坐标系下，这里是odom
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  //////////////////0. 记录时间扫描数据接收时间
  last_laser_received_ts_ = ros::Time::now();//用于检查laser消息多久没到（在另一个时间回调函数）不重要
  if( map_ == NULL ) {
    return;
  }
  ...
  ////////////////1.这里主要是设置激光传感器的的位姿（相对于机器人base_frame）,如果设置过则直接获取
  int laser_index = -1;
  // Do we have the base->base_laser Tx yet?　std::map< std::string, int > frame_to_laser_;
  if(frame_to_laser_.find(laser_scan->header.frame_id) == frame_to_laser_.end())
  {
    ...
    try
    {
      this->tf_->transformPose(base_frame_id_, ident, laser_pose);//得到的laser_pose为激光传感器相对于base_frame的位置
    }
    catch(tf::TransformException& e)
    {
  ...
  //////////////////　2.查询获取扫描数据时机器人的位姿，结合　与上一次扫描间的相对运动　以及　粒子滤波器的状态（是否已初始化）　判断是否更新粒子集
  ...
  //如果pf已初始化则根据当前观测与上次观测之间的运动距离判断是否更新粒子
  if(pf_init_)
  {
  ...
  //如果pf未初始化则需要更新粒子，并且发布位姿（force_publication）
  if(!pf_init_)
  {
  ...
  else if(pf_init_ && lasers_update_[laser_index])//作运动更新
  {
    //printf("pose\n");
    ...
    odom_->UpdateAction(pf_, (AMCLSensorData*)&odata); //运动更新
  ...
  {   
    ...
    if(!(++resample_count_ % resample_interval_))
    {
      ...
      pf_update_resample(pf_);  //粒子重采样（包含对粒子集重新聚类过程）
      resampled = true;
    }
